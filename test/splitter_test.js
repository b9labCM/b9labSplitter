...
contract('Splitter', function(accounts) {
	...
	beforeEach("should create a Splitter contract", function() {
  	...
  	it("should be 0 in the first account", function() {
    	...
    	return instance.getBalance.call(account_C);
   	        ...
   	        assert.equal(balance.toString(10), 0, " first account has not 0 balance");
   	 	});
  	});
	...
	/*Test 2: Initial balance B should be zero*/
  	it("should be 0 in the second account", function() {
    	    ...
    	    return instance.getBalance.call(account_D);
   	   		...
   	   		assert.equal(balance.toString(10), 0, " second account has not 0 balance");
   	 	});
  	});
	...
	it("testing split ether", function(){
	   	return Splitter.deployed().then(function(instance) {
		    ...
		    assert.equal(balance_C_before.toString(10), 0, " first account C before has not 0 balance");
			...
			assert.equal(balance_C_after.toString(10), splittedAmount, " account C has not the right amount of ether ");
			return inst.getBalance.call(account_D);
		}).then(balance =>  {
			balance_D_after = new BigNumber (balance);
			console.log(balance_D_after);
			assert.equal(balance_D_after.toString(10), splittedAmount, " account D has not the right amount of ether ");
		})
	});
